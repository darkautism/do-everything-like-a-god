...
async fn test_base64_encode() -> WebDriverResult<()> {
    ...
    assert!(textareas.len() >= 2, "Should have input and output textareas");
    ...
    assert!(!output.is_empty(), "Output should not be empty after encoding");
...
async fn test_theme_toggle() -> WebDriverResult<()> {
    ...
    assert!(class.unwrap_or_default().contains("light"), "Should have light theme");
